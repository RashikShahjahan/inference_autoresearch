...
- **`generate.py`** - the single file the agent edits. It contains the mutable batched generate-path implementation, currently adapted from `mlx_lm`.
...
The important baseline detail is that `generate.py` starts from a batched generation path adapted from `mlx_lm`, so experiments should optimize that implementation rather than replacing it with a simpler single-sample loop.
...
- **Single file to modify.** The agent only edits `generate.py`, which keeps the search space constrained and diffs easy to review even though the baseline is a larger batched decoder.
