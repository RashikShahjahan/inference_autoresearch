...
2. Run the `capture_gpu_trace` tool to capture a representative `batch_generate(...)` Instruments Metal trace.
3. Export and analyze the trace using `xctrace` and the `gpu-trace-analysis` skill.
